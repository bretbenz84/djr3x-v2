...
- Visual curiosity — when the conversation lulls with someone present, one of his options is to take a single look and warmly ask about a specific real detail he sees on them, the way real people do ("where'd you get that shirt?", "which side of the family's the blonde from?"). Grounded in an actual camera glance (never invented), person-focused, and safety-railed so it stays a friendly question about clothing/items/hair — never body, age, health, or looks (toggle with `VISUAL_CURIOSITY_ENABLED`)
- Waves back — when the camera sees someone wave at him (MediaPipe pose gesture), Rex returns the wave with his arm and a short warm line, the way you'd wave back across a room (debounced so one wave gets one wave-back; toggle with `WAVE_BACK_ENABLED`)
