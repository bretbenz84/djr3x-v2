...
- Live pose wireframe — the GUI dashboard's camera preview overlays the detected body as a real-time skeleton (MediaPipe pose landmarks) on top of the face boxes, so you can see what Rex's body-tracking sees (toggle with `GUI_POSE_WIREFRAME_ENABLED`)
