"""
Tool-calling router (intelligence/tool_router.py — docs/tool_router_scope.md).

Pins the catalog contracts: every ActionSpec has a tool definition (a new action
without one fails HERE, not silently at runtime), names round-trip, and
conversation.reply is the no-tool default. The live-cutover tests below pin the
subset that rides the lean reply call and how its tool calls dispatch.
"""
