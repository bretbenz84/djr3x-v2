...
> **STATUS (2026-06-22, commit `dfffab3`):** 9 conversation-quality items are **✅ FIXED** with regression tests — ranks **4** (interest poisoning), **7** (cross-session voice→name), **9** (force_salient governor), **10** (people_roast/memory_musing priority), **11** (gentle_probe tender), **14** (onboarding pivot), **17** (reactive-callback 'light' frame), **25** (one-word answer ≠ soft), **28** (reveal cadence). The games items (1, 6, 21, 22) were intentionally deferred (untested area). All touched test modules pass in isolation; the full `unittest discover` run has **pre-existing nondeterministic cross-module isolation flakiness** (present at baseline `b2c3e5c` — verify per-module, not via one discover run).
