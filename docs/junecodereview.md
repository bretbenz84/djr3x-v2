...
At review time: `1942 tests, 2 failures`. Now **1 of the 2 is fixed**:

1. ✅ **FIXED (commit `99d7825`) — Comedy regression.** `test_collapses_overexplained_joke_tail` — `intelligence/comedy_modes.py` `_collapse_overexplained_joke` now also strips a sentence-initial `"Get it?"` explainer (lookbehind keeps the punchline's own terminal punctuation), so `"That was a great landing. Get it? Because I crashed."` → `"That was a great landing."` while `"I can't see you."` / `"Let me get it for you."` stay untouched. Rex stops explaining his own jokes.
2. ⬜ **Still red — responsiveness tuning drift (low goal impact).** `test_live_tracked_edge_face_slews_neck_responsively` (`tests/test_face_tracking.py:836`) — neck slew `144` vs expected `≥150`. Either a face-tracking gain regressed or the threshold is stale; decide which is correct on-device.
...
> **STATUS (2026-06-22, commit `dfffab3`):** 9 conversation-quality items are **✅ FIXED** with regression tests — ranks **4** (interest poisoning), **7** (cross-session voice→name), **9** (force_salient governor), **10** (people_roast/memory_musing priority), **11** (gentle_probe tender), **14** (onboarding pivot), **17** (reactive-callback 'light' frame), **25** (one-word answer ≠ soft), **28** (reveal cadence). **Since then also FIXED:** **#13** (multi-person pose/face, `30ace1c`), **#3** (per-session callback recall) + **#5** (don't analyze own DJ music) (`932ad97`), **#12** (web-search model fallback) + **#2** (fun commands not swallowed after a question) + **#35** (unmapped explicit commands route) (`cb625e1`), and **#18** (POV near-verbatim repeat guard) + **#19** (streaming surprise pre-beat) + the §1 joke-tail comedy regression (`99d7825`). **Engagement + remembers + safety batch:** **#16** (world-reaction dedupe before random.choice), **#20** (deterministic third-party callback backstop), **#15** (postponed events kept open, not lost), **#30** (riff-on-the-room cooldown stamped only on success), **#37** (forget-by-target can't wipe stores via metadata columns) — all with regression tests. The games items (1, 6, 21, 22) were intentionally deferred (untested area). All touched test modules pass in isolation; the full `unittest discover` run has **pre-existing nondeterministic cross-module isolation flakiness** (present at baseline `b2c3e5c` — verify per-module, not via one discover run).
...
**2. ✅ FIXED (commit `cb625e1`) — Humor/game/performance commands swallowed for 120s after any Rex question** — bug / high / high · `intelligence/dialogue_act.py:326-345` + `intelligence/interaction.py:1349-1380` (`_dialogue_allows_action_breakout`), `:18461`, `:18497`
...
**3. ✅ FIXED (commit `932ad97`) — Episodic & nostalgia callback dedup sets never cleared per session → fire once per BOOT not per conversation** — bug / high / high · `intelligence/llm.py:344,348,352,393,436,463`; `intelligence/interaction.py:13223-13260` (`_end_session`)
...
**4. ✅ FIXED — Interest regex mints garbage interests from auxiliary "I do …" and persists them at confidence 0.95** — bug / high / high · `intelligence/conversation_steering.py:73` (pattern), `:441-468` (`_store_interest_fact`), `:232-244` (`note_user_turn`)
...
**5. ✅ FIXED (commit `932ad97`) — Scene analyzer runs during DJ music → Rex startles/laughs at his own playback** — bug / high / high · `audio/scene.py:65-85` (skip guard), `:114-122`; consumed at `intelligence/consciousness.py:4808-4839` and `intelligence/llm.py:243-247`
...
**7. ✅ FIXED — Cross-session voice→person resolution not implemented — named, departed speaker returns as unknown_voice_N** — ineffective_feature / medium / high · `intelligence/interaction.py:2519-2549` (`_resolve_anonymous_speaker_slot`); `memory/voice_signatures.py:111-147` (match), `:199-219` (`attach_person`)
...
**9. ✅ FIXED — Governor drops force_salient candidates (animal arrival starved)** — ineffective_feature / medium / medium · `intelligence/speech_engine.py:343-392,415-417`; `intelligence/action_governor.py:358,403,407`
...
**10. ✅ FIXED — people_roast and memory_musing default to floor priority 20** — ineffective_feature / low / medium · `intelligence/action_governor.py:74-105,304`; `intelligence/idle_behaviors.py:138-145,287-300`
...
**11. ✅ FIXED — gentle_probe (masked-distress) mode still allows roasting and visual jabs** — bug / medium / medium · `intelligence/social_frame.py:848` (`_roast_level`), `:779` (`_visual_allowed`); directive at `intelligence/empathy.py:745-750`
...
**12. ✅ FIXED (commit `cb625e1`) — Web search runs on gpt-5.4-mini with unverified hosted web_search tool support** — ineffective_feature / high / high · `intelligence/web_search.py:83-88` (`_search_model`), `406-449` (`answer`); `config.py:4625` (`WEB_SEARCH_MODEL=None`), `170` (`LLM_CONVERSATION_MODEL`)
...
**13. ✅ FIXED (commit `30ace1c`) — Multi-person gesture/wave mis-attribution — pose always writes slot 0** — bug / medium / medium · `vision/pose.py:547-597` (`_update_world_state`); `intelligence/consciousness.py:2582-2604` (`_step_wave_reaction`)
...
**14. ✅ FIXED — Onboarding is_pivot misfires on enthusiastic answers** — bug / medium / medium · `intelligence/onboarding.py:416-448` (`is_pivot`/`_PIVOT_PAT`), called at `intelligence/interaction.py:9142-9146`
...
**15. ✅ FIXED — "Postponed" treated as a hard cancellation — a rescheduled event is durably lost** — bug / low / medium · `memory/events.py` (`_POSTPONE_PAT` + `looks_like_postponement` split out of `_CANCEL_PAT`; new `reschedule_event` keeps the plan OPEN and clears the stale date; new `postpone_matching_events`); `intelligence/interaction.py` wires both the follow-up-response and proactive paths. Tests: `tests/test_event_cancellation.py`, `tests/test_event_postponement.py`.
...
**16. ✅ FIXED — World-reaction triggers consume dedupe state before random.choice** — bug / medium / medium · `intelligence/consciousness.py` `_step_proactive_reactions` — the notable-date / time-of-day / weather blocks now stash their dedupe key in trigger metadata (`ack_date`/`ack_tod`/`ack_weather_signature`) and only consume it for the trigger actually chosen by `random.choice`; added the missing `_acknowledged_dates.clear()` to session reset. Un-chosen co-occurring triggers stay retryable. Test: `tests/test_world_reaction_dedupe.py`.
...
**17. ✅ FIXED — Reactive callbacks require roast level exactly 'normal'** — ineffective_feature / low / medium · `intelligence/callback_engine.py:714-719`; `intelligence/social_frame.py:867-880` (`_roast_level`)
...
**18. ✅ FIXED (commit `99d7825`) — Rex-POV double-utterance guard never arms on the normal reply path** — ineffective_feature / medium / medium · `intelligence/rex_pov.py:297-318`; `intelligence/interaction.py:4180-4186` (only caller); `intelligence/llm.py:1086-1089`
...
**19. ✅ FIXED (commit `99d7825`) — Surprise pre-beat is effectively dead on the live streaming reply path** — ineffective_feature / medium / medium · `intelligence/interaction.py:9644-9655, 10140-10149, 9982-9995`; `intelligence/llm.py:1283-1311` (`classify_surprise`)
...
**20. ✅ FIXED — Third-party/secondhand premise exclusion is only a soft LLM instruction** — ineffective_feature / medium / medium · `intelligence/callback_engine.py` — new deterministic `_looks_like_thirdparty_premise` backstop in `bank_from_turn` rejects family/friend possessives ("my brother") and third-person hobby clauses ("he collects …") before banking, so Rex can't roast the user about someone else's hobby. Conservative (drops rather than risks). Tests: `tests/test_callback_humor.py` (`BankerTests`).
...
**25. ✅ FIXED — is_soft_disengage treats any one-word answer as lukewarm** — ineffective_feature / low / medium · `intelligence/onboarding.py:451-460` (`is_soft_disengage`), consumed at `interaction.py:9152-9168` and `onboarding.py:315,371`
...
**28. ✅ FIXED — Onboarding self-reveal (reciprocity beat) fires at most ~once** — ineffective_feature / low / medium · `intelligence/interaction.py:9095,9183-9193`; `config.py:3411` (`ONBOARDING_REVEAL_EVERY=3`), `3394` (MAX=5)
...
**30. ✅ FIXED — do_bored_environment_snark / do_live_vision_comment burn their cooldown even when speech is blocked** — bug / low / medium · `intelligence/idle_behaviors.py` — the cooldown timestamp now stamps INSIDE the worker, after `_can_proactive_speak()` passes AND a line is produced, so a blocked gate / empty scene no longer eats the full 4-5 min cooldown on a no-op. Test: `tests/test_idle_behaviors_cooldown.py`.
...
**35. ✅ FIXED (commit `cb625e1`) — Legacy commands not in the action map unconditionally blocked during answer_to_rex** — bug / medium / medium · `intelligence/interaction.py:1242-1259` (`_legacy_command_blocked_by_dialogue`)
...
**37. ✅ FIXED — Forget-by-target matches against fact source/category fields** — bug / low / low · `memory/forgetting.py` — dropped the structural columns (`source`/`category`/`domain`/`preference_type`/`interest_strength`) from the searchable field tuples in `forget_specific_memory`, `forget_memory_detail`, and `fact_or_event_matches`, so "forget all explicit memories" can no longer wipe whole stores by matching metadata. Content matching ("forget jazz") still works. Test: `tests/test_forget_metadata_isolation.py`.
...
## 3. CONTEXT.md corrections — STATUS: ALL DOC/COMMENT FIXES APPLIED

Confirmed claim-vs-code mismatches. **UPDATE (2026-06-21): every documentation/comment correction below is now applied**, across three commits — CONTEXT.md (`cedd1ae`), README.md + this review doc (`d07d9ca`), and the in-code docstring/comment cleanup (C7/C8 code-side halves, C9, C10, C12, C13, C16, C17, C18-`supervisor.md`, C19, C20, C21). So `CONTEXT.md`, `README.md`, `docs/supervisor.md`, and the cited module docstrings/comments now all match the code.

**What remains is CODE behavior, not docs** — the docstrings now describe these honestly, and the actual fixes are tracked as Part-1/Part-2 findings, not here:
- **C8** — `question_budget.py:230` getattr fallback is still `2` (live config is 5); harmless unless `config.py` is stripped.
- **C17** — onboarding step TTL is still wall-clock-since-armed (could be made to slide on `asked_at`).
- **C19** — the `submit_text` pause bypass (Part-1 finding #8) is still a live bug; the code comment now flags it.
- **C20** — session-end consolidation still records emotional-event recency as `unknown` (could resolve it).
- **C9 / C7** — the gaze SPEAKING duty-cycle + multi-person include-sweep remain inert on the live robot (a wire-or-drop decision); the docstring now says so.

**Status key:** ✅ in the `#` column = the doc/comment claim is now corrected. The table below is the original detail.
...
| C3 ✅ (code now fixed) | L356-358: a named-then-departed voice resolves "straight to them" next session via `attach_person` | Originally the READ side was unwired (returned an anonymous label). **Now fixed (finding #7, commit `dfffab3`):** `_resolve_anonymous_speaker_slot` resolves a confident person-linked signature straight to that person. CONTEXT.md updated to describe the wired read side (`VOICE_SIGNATURE_RESOLVE_PERSON_*`). | Done — doc and code now agree. |
...
| C8 ✅ (docs; code fallback still 2) | L494 + `onboarding.py:4`: `QUESTION_BUDGET_MAX_QUESTIONS=3`/90s | `config.py:3357` sets **5**; `question_budget.py:230` getattr fallback is a third value (2) | Update to 5/90s; align the getattr fallback to 5. |
...
| C11 ✅ (README) | README L22: web-search trigger phrases "editable in your user config" | live gitignored `user_config.py` predates web search and lacks the entire WEB SEARCH section; `setup_macos.sh:415-417` never re-copies an existing file | Re-sync `user_config.py` from `user_config.example.py` (or document that users must re-copy new template sections). Runtime is unaffected (config.py defaults stand). |
