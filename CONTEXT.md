...
- Proactive-speech "yield the floor" guard (do not regress; fixes "Rex talked over me right as I started answering"): live run — Rex asked "What's the hardest thing you've had to deal with?", the user paused to think, and idle banter fired and played over the start of their answer; the user then couldn't interrupt (half-duplex). Root cause is NOT a too-short wait timer — it's a **race/lag**. A proactive line is decided on a stale silence timer (`idle_for`, measured from when *Rex* stopped via `_apply_post_tts_handoff` setting `_last_speech_at`), and then the path runs LLM + TTS **inline/blocking** (`_speak_blocking`) for ~1-2s. The interaction VAD loop (`_CHUNK_SECS=0.032`, normally ~31 checks/s) is paused that whole window, so a user who begins talking during generation/TTS-fetch is invisible; by the time audio plays the user is mid-sentence and AEC then masks them. KEY insight: checking VAD just before `_speak_blocking` does NOT fix it — `_speak_blocking` still pays the ~1s ElevenLabs time-to-first-stream after the check, re-opening the gap. So the fix **pre-caches the TTS, THEN re-checks the mic immediately before the (now-instant) playback, and aborts if the user is already talking.** `audio/barge_guard.user_speaking_now()` scans the last `PROACTIVE_SPEECH_YIELD_WINDOW_SECS` (0.6) of the **un-attenuated** rolling buffer with `vad.get_speech_segments` and returns True when ≥ `PROACTIVE_SPEECH_YIELD_MIN_SPEECH_SECS` (0.1) is speech; it returns False while `echo_cancel.is_suppressed()` (the buffer would hold Rex's own voice). `interaction._speak_proactive(text, emotion, …)` wraps `tts.ensure_cached` → `user_speaking_now()` → `_speak_blocking` (abort = return False, no speech) and now backs idle banter / interest idle follow-up / low-memory idle question / idle outro; `consciousness._speak_async` does the same `ensure_cached` + `user_speaking_now` check before its `priority=0` enqueue (covers proactive greetings/check-ins/small-talk). On abort the line isn't spoken and the un-attenuated buffer lets the normal turn loop capture the user; the per-path "already said this" bookkeeping (`_idle_banter_count`, `_interest_idle_followups_spoken`, `_low_memory_idle_questions_spoken`, `_idle_outro_spoken`) was moved into the `if completed:` blocks so a yield doesn't consume it. `audio/vad.py` now serializes all model access with a lock (`_lock`) because the guard runs on the consciousness thread concurrently with the interaction VAD loop — a `get_speech_segments` reset/inference must not corrupt the streaming `is_speech` state. SCOPE/LIMIT: this is NOT true barge-in — it only catches speech that began **before/at** the moment Rex starts (the common case the user hit). Speech that starts **deep into** Rex's playback still hits the half-duplex/AEC wall and needs the hardware AEC path. Direct replies (the user just spoke) are deliberately NOT guarded — only self-initiated speech. Gated by `config.PROACTIVE_SPEECH_YIELD_ENABLED` (default True); hardware/room-sensitive (false-positive yields → raise `PROACTIVE_SPEECH_YIELD_MIN_SPEECH_SECS`; missed yields → widen the window). Do NOT move the mic-check before the `ensure_cached` pre-cache (that re-opens the TTS lag). Tests: `tests/test_barge_guard.py`; idle-path tests in `test_audio_and_conversation_gating.py` now mock `tts.ensure_cached` + `barge_guard.user_speaking_now`.
