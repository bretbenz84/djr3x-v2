...
- Mood-driven body language (`intelligence/body_mood.py`, pure state): a decaying "body mood" set by compliments/insults/amusement shapes posture — head lift/tilt bias on the RESTING pose (never fights the face-centering controller), visor openness, breathing cadence, occasional idle gesture. Camera repositioned 2026-09-25: visor lens-clear floor is 5100 q-µs (1275 µs), reserved for suspicion/anger/offense; mood decay returns toward neutral (1525 µs), with idle drift from 1480–1570 µs between speech/listening. Ordinary speech starts at 1385 µs; surprise/shock retains full opening (1744 µs), while camera captures use 1680 µs. Both lower-corner detection masks are now empty. `consciousness._step_mood_expression`/`_mood_rest_bias`; `tests/test_body_mood.py`.
...
- Jeopardy spoken-answer judging rework (2026-07-07): players answer by VOICE, and the
  ...
  The per-turn spoken "Remaining categories: …" reminder follows the same
  fatigue curve in BOTH modes (full for the first
  `JEOPARDY_CATEGORIES_REMINDER_FULL_READS` scoring turns, then every
  `..._EVERY`-th). The old blanket GUI mute is now opt-in only
  (`JEOPARDY_READ_CATEGORIES_WITH_GUI=False`, for a table sitting at the
  screen) — as a default it silently killed the read-out for players sitting
  around the ROBOT, and the 2026-08-26 game spoke none at all. The
  once-per-round fresh-board announcement is unchanged.
