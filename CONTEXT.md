...
- Supervisor wake detection (do not regress to ONNX-only): the `wakeuprex.onnx` model is unreliable and often never crosses threshold (this caused "I said wake up Rex and nothing happened" on the robot). The MAIN APP doesn't rely on it either — from SLEEP it wakes via VAD + local Whisper transcription matching the phrase (`interaction._is_sleep_wake_transcript`, `SLEEP_TRANSCRIBED_WAKE_FALLBACK_ENABLED`), NOT the ONNX score. So `rex_supervisor.py` now defaults to `REX_SUPERVISOR_WAKE_MODE=both`: a loudness (RMS) gate captures a spoken phrase, `mlx-whisper` (local model in `assets/models/whisper/`) transcribes it, and `_transcript_is_wake_phrase` matches "wake up rex"/variants — in parallel with the ONNX score. IMPORTANT: it gates on RMS, NOT Silero VAD — per-80 ms `get_speech_timestamps` calls return nothing (that API needs a longer window to find a start+end segment), which silently broke the first version (mic worked per `--meter`, but no phrase ever accumulated). Self-contained (no project `config` import; reuses mlx_whisper + the local model dir directly). Knobs: `REX_SUPERVISOR_RMS_GATE` (0.012; raise for noisy rooms, lower for quiet mics — `--meter` shows your level), `REX_SUPERVISOR_WAKE_THRESHOLD` (onnx). Emits a `[diag]` line every 5s with mic RMS + gate + accumulated-chunk count + peak ONNX. Modes: `transcribe` (reliable, default-on path), `onnx` (score only), `both`. Tests: `SupervisorWakePhraseTest`.
