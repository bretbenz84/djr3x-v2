...
- Visual curiosity question (`intelligence/idle_behaviors.do_visual_curiosity_question`, `VISUAL_CURIOSITY_*`; `vision.scene.describe_person_detail`; `tests/test_visual_curiosity.py`): a PERSON-focused idle option (one of the `_idle_micro_behavior_choices` people-present slots) — Rex takes ONE GPT-4o look and warmly ASKS about a specific real detail he sees on the present person ("where'd you get that shirt?", "which side of the family's the blonde from?"). Distinct from bored_env_snark (room, complaint-framed) and live_vision_comment (observation, no question) and appearance_riff (stored facts, no live look). Targets the engaged / sole-visible KNOWN person; never invents a detail; hard-cooldowned (one vision call). Safety: `describe_person_detail` is railed to clothing/items/accessories/hair ONLY — never body/weight/age/health/ethnicity/disability/attractiveness (prompt rules + `_PERSON_DETAIL_BANNED` backstop). Boundary-aware (`boundaries.is_blocked` appearance/clothing) and yields to the give-space-after-heavy gate via `_can_proactive_speak`. Asks via `_generate_and_speak(purpose="visual_curiosity")` so the answer binds normally.
