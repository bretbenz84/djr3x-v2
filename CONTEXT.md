...
- Supervisor wake detection (do not regress to ONNX-only): the `wakeuprex.onnx` model is unreliable and often never crosses threshold (this caused "I said wake up Rex and nothing happened" on the robot). The MAIN APP doesn't rely on it either — from SLEEP it wakes via VAD + local Whisper transcription matching the phrase (`interaction._is_sleep_wake_transcript`, `SLEEP_TRANSCRIBED_WAKE_FALLBACK_ENABLED`), NOT the ONNX score. So `rex_supervisor.py` now defaults to `REX_SUPERVISOR_WAKE_MODE=both`: Silero VAD accumulates a spoken phrase, `mlx-whisper` (local model in `assets/models/whisper/`) transcribes it, and `_transcript_is_wake_phrase` matches "wake up rex"/variants — in parallel with the ONNX score. Self-contained (no project `config` import; reuses silero_vad + mlx_whisper + the local model dir directly). Emits a `[diag]` line every 5s with peak ONNX score + mic RMS for debugging. Modes: `transcribe` (reliable, default-on path), `onnx` (score only), `both`. Tests: `SupervisorWakePhraseTest`.
